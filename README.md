...
- **Lambda Function:** part that does all the work to format events for
  publishing, deployed from the `deploy_localzip` path or `deploy_s3zip`.
...
These variables configure specific parameters for the Lambda's operation. This
is most likely what you want to customize.
...
These variables control where the Lambda Function logs are sent. You will
always have a copy of the logs in CloudWatch Logs, but you can also have a copy
sent to another Lambda Function or a Log Destination (for cross-account
logging).
...
Take a look at the `terraform/1.0-aws4/example` directory for how to use this
module.
